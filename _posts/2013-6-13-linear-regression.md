...
![y = mx + b](http://l.wordpress.com/latex.php?latex=y%20%3D%20mx%20%2B%20b&bg=FFFFFF&fg=470229&s=1 "y = mx + b")
...
![y = b1x1 + b2x2 +...+ bnxn + e](http://l.wordpress.com/latex.php?latex=y%20%3D%20b1x1%20%2B%20b2x2%20%2B...%2B%20bnxn%20%2B%20e&bg=FFFFFF&fg=470229&s=1 "y = b1x1 + b2x2 +...+ bnxn + e")
...
![h(x) = b1x1 + b2x2 +...+ bnxn + e](http://l.wordpress.com/latex.php?latex=h%28x%29%20%3D%20b1x1%20%2B%20b2x2%20%2B...%2B%20bnxn%20%2B%20e&bg=FFFFFF&fg=470229&s=1 "h(x) = b1x1 + b2x2 +...+ bnxn + e")

We like to use matrices to make computation more efficient, and so typically we write this as a ![XTB](http://l.wordpress.com/latex.php?latex=XTB&bg=FFFFFF&fg=470229&s=1 "XTB"), where X is a vector of explanatory variables, and B is a vector of coefficients.  The errors are captured by a vector represented with the **e **term.  Note that this equation is just representing one outcome (y) with one set of features (x's).
...
<div class="wp-caption alignleft" style="width: 510px">[![](http://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Linear_regression.svg/500px-Linear_regression.svg.png)](http://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Linear_regression.svg/500px-Linear_regression.svg.png)Simple linear regression with one independent variable
...
In this case, we are summing from 1 to m, where m is the number of cases we have in our dataset, each with one outcome variable (y) and a set of (x).  This equation represents exactly what we discussed above - ![h(x)](http://l.wordpress.com/latex.php?latex=h%28x%29&bg=FFFFFF&fg=470229&s=1 "h(x)") is our model's prediction of the outcome based on the features, ![y(i)](http://l.wordpress.com/latex.php?latex=y%28i%29&bg=FFFFFF&fg=470229&s=1 "y(i)") is the real outcome, and so ![h(x)i) - y(i)i](http://l.wordpress.com/latex.php?latex=h%28x%29i%29%20-%20y%28i%29i&bg=FFFFFF&fg=470229&s=1 "h(x)i) - y(i)i") is the difference between the actual and observed outcome, our error, which we square, and then sum up for all of our m data points.  This equation gives rise to the **ordinary least squares regression model.**
...
![xj = xj + a (h(x)i) - y(x)i)^2 * xj](http://l.wordpress.com/latex.php?latex=xj%20%3D%20xj%20%2B%20a%20%28h%28x%29i%29%20-%20y%28x%29i%29%5E2%20%2A%20xj&bg=FFFFFF&fg=470229&s=1 "xj = xj + a (h(x)i) - y(x)i)^2 * xj")
...
![X = (XTX)^(-1)XTy](http://l.wordpress.com/latex.php?latex=X%20%3D%20%28XTX%29%5E%28-1%29XTy&bg=FFFFFF&fg=470229&s=1 "X = (XTX)^(-1)XTy")

where X is our matrix of features (the transpose is like squaring it), y is the vector of observed outcomes (remember this is supervised learning so we have labels!)  If you want the lovely derivation, I suggest you take CS229 Machine Learning.  ![:)](http://www.vbmis.com/learn/wp-includes/images/smilies/simple-smile.png)  You can plug that into Matlab and get the answer with one line, no looping needed!  That is pretty awesome.
